...
function convertCases(nodes: Array<Object>, context: Context, needle: Identifier): Expression {
  if (nodes.length === 0) {
    return t.identifier('undefined');
  }
  const node = nodes[0];
  const consequent = context.staticBlock((childContext: Context): Expression => {
    const children = node.block.nodes.map(node => visitExpression(node, childContext));
  ...
  const test = t.binaryExpression('===', needle, parseExpression(node.expr, context));
...
const ConditionalVisitor = {
  expression(node: Object, context: Context): Expression {
    ...
    const id = t.asIdentifier(needle) || context.declareVariable('const', context.generateUidIdentifier('case_variable').name).id;
    const cases = convertCases(node.block.nodes, context, id);
    if (!t.isIdentifier(needle)) {
      return t.sequenceExpression([t.assignmentExpression('=', id, needle), cases]);
    }
    return cases;
  },
};
